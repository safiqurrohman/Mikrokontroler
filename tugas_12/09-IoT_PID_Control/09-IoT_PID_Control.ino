...
const char* ssid = "hastagButuhWifi"; // Enter your WiFi name
const char* password =  "Hilman31!"; // Enter WiFi password
...
//    if (client.connect("ESP32Client", mqttUser, mqttPassword )) {
    ...
    client.subscribe("autoSet_hilman");
    client.subscribe("SetPoint_hilman");    
    client.subscribe("Nilai_Kc_hilman");
    client.subscribe("Nilai_tauI_hilman");
    client.subscribe("Nilai_tauD_hilman");
  }
}
...
void loop() { 
  ...
  if(Topic=="autoSet_hilman"){
    autoSet=Payload.toInt();
  }
   if(Topic=="Nilai_Kc_hilman"){
    Kc=Payload.toFloat();
  }
   if(Topic=="Nilai_tauI_hilman"){
    tauI=Payload.toFloat();
  }
   if(Topic=="Nilai_tauD_hilman"){
    tauD=Payload.toFloat()/6;
  }
   if(Topic=="SetPoint_hilman"){
    sp=Payload.toFloat();
  }
  Serial.println("<-------------------------->");
  ...
  client.publish("Suhu1_hilman",suhu1);

  dtostrf(cel1, 1, 0, suhu2);
  client.publish("Suhu2_hilman",suhu2);

  dtostrf(sp, 1, 0, Tampil_SP);
  client.publish("Tampil_SP_hilman",Tampil_SP);

  dtostrf(Kc, 1, 0, Tampil_Kc);
  client.publish("Tampil_Kc_hilman",Tampil_Kc);

  dtostrf(tauI, 1, 0, Tampil_tauI);
  client.publish("Tampil_tauI_hilman",Tampil_tauI);

  dtostrf(tauD, 1, 0, Tampil_tauD);
  client.publish("Tampil_tauD_hilman",Tampil_tauD);  
  ...
  if(autoSet==1){
    ...
    // bisa diubah2, atau sesuai yg muncul terakhir
     ...
     client.publish("Nilai_op_hilman",Nilai_op);
  }
}
